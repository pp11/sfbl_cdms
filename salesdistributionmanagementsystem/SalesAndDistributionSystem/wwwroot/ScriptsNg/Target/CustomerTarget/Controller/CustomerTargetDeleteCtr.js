ngApp.controller('ngGridCtrl', ['$scope', 'CustomerTargetService', 'permissionProvider', 'gridregistrationservice', 'notificationservice', '$http', '$log', '$filter', '$timeout', '$interval', '$q', function ($scope, CustomerTargetService, permissionProvider, gridregistrationservice, notificationservice, $http, $log, $filter, $timeout, $interval, $q) {
    ...
    //    }
    //    , {
    //        name: 'TARGET_QTY', field: 'TARGET_QTY', displayName: 'Target Qty', enableFiltering: true, enableColumnMenu: false, cellTemplate: '<input type="number"  style="text-align: left;"  ng-model="row.entity.TARGET_QTY" min="0" class="pl-sm" ng-change="grid.appScope.CalculateValue(row.entity)" />'
    //    }
    //    , {
    ...
    //    }
    //    , {
    //        name: 'Action', displayName: 'Action', enableFiltering: true, enableColumnMenu: false, cellTemplate:
    ...
    $scope.GetEditDataById = (id) => {
        if (id != "" && id != null && id != 0) {
            CustomerTargetService.GetEditDataById(id).then(response => {
                //$scope.model = response.data;
                $scope.model.MST_ID = response.data?.MST_ID;
                $scope.model.YEAR = response.data?.YEAR;
                $scope.model.CUSTOMER_ID = response.data?.CUSTOMER_ID.toString();
                //$scope.OnChangeCustomer();
                $scope.model.MONTH_CODE = response.data?.MONTH_CODE.toString();
                $scope.TriggerSelects();
                var rowNo = 1;
                response.data?.TARGET_DTLs.forEach(e => {
                    e.SKU_ID = e.SKU_ID.toString();
                    e.ROW_NO = rowNo++;
                });
                $scope.gridOptionsList.data = [$scope.DefaultData(), ...response.data?.TARGET_DTLs];
            })
        }
    }
    ...
    $scope.ClearForm = () => {
        window.location = "/Target/CustomerTarget/CustomerNationalTarget";
    }
    ...
    $scope.SaveData = function (model) {
        ...
        if (model.YEAR == null && model.YEAR == '') {
            notificationservice.Notification("Enter Year", "", 'Enter Year');
            return;
        }
        if (model.MONTH_CODE == null || model.MONTH_CODE == '' || model.MONTH_CODE == '00') {
            notificationservice.Notification("Enter Month", "", 'Enter Year');
            return;
        }
        // Confirmation dialog before proceeding with deletion
        var confirmDelete = confirm("Are you sure you want to delete the target of " + model.MONTH_CODE + "/" + model.YEAR + "? This action cannot be undone.");
        if (!confirmDelete) {
            return; // Cancel action if user selects 'Cancel' in the confirmation dialog
        }

        $scope.showLoader = true;
        CustomerTargetService.AddOrUpdate(model).then(function (data) {
            if (data.data?.status == 1) {
                ...
                notificationservice.Notification(1, 1, 'target data delete successfully!');
            }
            else {
                $scope.showLoader = false;
                //$scope.addDefaultRow($scope.GridDefalutData());
