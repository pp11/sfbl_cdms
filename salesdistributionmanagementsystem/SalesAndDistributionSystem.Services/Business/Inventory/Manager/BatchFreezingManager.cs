...
{
    public class BatchFreezingManager : IBatchFreezingManager
    {

        private readonly ICommonServices _commonServices;
        private readonly IConfiguration _configuration;
        public BatchFreezingManager(ICommonServices commonServices, IConfiguration configuration)
        {
        ...
        string GetBatchNo_Query() => @"SELECT DISTINCT 
                                            ...
                                            (SELECT PASSED_QTY FROM BATCH_WISE_STOCK WHERE COMPANY_ID=A.COMPANY_ID AND UNIT_ID=A.UNIT_ID AND SKU_ID=A.SKU_ID AND BATCH_ID=A.BATCH_ID)STOCK_QTY,
                                              A.PASSED_QTY BATCH_QTY,
                                              A.PASSED_QTY BATCH_FREEZE_QTY
        ...
        string GetMstData_Query() => @"SELECT MST_ID, 
                                               ...
                                               FN_PACK_SIZE(COMPANY_ID,SKU_ID) PACK_SIZE, 
                                               REMARKS
                                        FROM BATCH_FREEZING_MST
                                        WHERE COMPANY_ID = :param1
                                        AND   UNIT_ID = :param2
                                        ORDER BY MST_ID DESC";


        string InsertQueryMst() => @"INSERT INTO BATCH_FREEZING_MST(
                                                                    MST_ID, 
                                                                    ENTRY_DATE, 
                                                                    COMPANY_ID, 
                                                                    UNIT_ID, 
        ...
        string InsertQueryDtl() => @"INSERT INTO BATCH_FREEZING_DTL 
                                                                   (
                                                                    DTL_ID, 
                                                                    MST_ID,
                                                                    FREEZE_DATE,
                                                            ...
                                                            VALUES (:param1, 
                                                                    ...
                                                                    :param12,                                                                     
                                                                    :param13,
                                                                    TO_DATE(:param14, 'DD/MM/YYYY HH:MI:SS AM'), 
                                                                    :param15
                                                                     
                                                                    )";

        string UpdateQueryMst() => @"UPDATE BATCH_FREEZING_MST SET  
                                                                  REMARKS=:param1,
                                                                  UPDATED_BY= :param2,
        ...
        string UpdateQueryDtl() => @"UPDATE BATCH_FREEZING_DTL SET FREEZE_QTY = :param1, 
                                                                   UPDATED_BY= :param2,
                                                                   UPDATED_DATE=  TO_DATE(:param3, 'DD/MM/YYYY HH:MI:SS AM'),
                                                                   UPDATED_TERMINAL= :param4
                                                                   WHERE DTL_ID = :param5";
        ...
        string GetMaxMstIdQuery() => "SELECT NVL(MAX(MST_ID),0)+1 MST_ID FROM BATCH_FREEZING_MST";
        string GetMaxDtlIdQuery() => "SELECT NVL(MAX(DTL_ID),0)+1 DTL_ID FROM BATCH_FREEZING_DTL";

        public async Task<string> InsertOrUpdate(string db, BatchFreezingMst model)
        {
            if (model == null)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (var item in model.batchFreezingDtlList)
                        {
                            ...
                            {
                                listOfQuery.Add(_commonServices.AddQuery(UpdateQueryDtl(), _commonServices.AddParameter(new string[] {  item.FREEZE_QTY.ToString(),
                                                                                                                                        model.UPDATED_BY.ToString(),
                                                                                                                                        model.UPDATED_DATE,
