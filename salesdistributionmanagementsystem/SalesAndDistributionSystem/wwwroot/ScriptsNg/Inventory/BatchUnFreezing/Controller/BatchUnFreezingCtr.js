ngApp.controller('ngBatchFreezingCtrl', ['$scope', 'uiGridConstants', 'BatchUnFreezingServices', 'notificationservice', 'gridregistrationservice', '$http', '$log', '$filter', '$timeout', '$interval', '$q', function ($scope, uiGridConstants, BatchUnFreezingServices, notificationservice, gridregistrationservice, $http, $log, $filter, $timeout, $interval, $q) {
    ...
    $scope.ClearEntity = function () {
        ...
        $scope.gridBatchFreezing.data[0].BATCH_FREEZE_QTY = 0;
    ...
    $scope.GetCompanyId = function () {
        ...
        BatchUnFreezingServices.GetCompanyId().then(function (data) {
    ...
    $scope.GetCompanyName = function () {
        $scope.showLoader = true;
        BatchUnFreezingServices.GetCompanyName().then(function (data) {
    ...
    $scope.GetUnitId = function () {
        $scope.showLoader = true;
        BatchUnFreezingServices.GetUnitId().then(function (data) {
    ...
    $scope.GetUnitName = function () {
        $scope.showLoader = true;
        BatchUnFreezingServices.GetUnitName().then(function (data) {
    ...
    $scope.GetSkuList = function () {
        ...
        BatchUnFreezingServices.GetSkuList($scope.model.COMPANY_ID, $scope.model.UNIT_ID).then(function (data) {
    ...
    $scope.OnSelectSkuName = function () {
        ...
        BatchUnFreezingServices.GetBatchList($scope.model.COMPANY_ID, $scope.model.UNIT_ID, $scope.model.SKU_ID).then(function (data) {
    ...
    var BatchFreezingcolDefs = [
        ...
        {
            name: 'FREEZE_DTL_ID', field: 'FREEZE_DTL_ID', displayName: 'Batch No', enableFiltering: true, width: '15%', cellTemplate:
                '<select class="select2-single form-control batch-no"  ng-style="row.entity.ROW_NO != 0 && grid.appScope.myObj" ng-disabled="row.entity.ROW_NO !=0"   data-select2-id="{{row.entity.FREEZE_DTL_ID}}" id="FREEZE_DTL_ID"' +
                'name="FREEZE_DTL_ID" ng-model="row.entity.FREEZE_DTL_ID" style="width:100%" ng-change="grid.appScope.OnSelectBatchNo(row.entity)">' +
                '<option ng-repeat="item in grid.appScope.Batches" ng-selected="item.FREEZE_DTL_ID == row.entity.FREEZE_DTL_ID" value="{{item.FREEZE_DTL_ID}}"> {{ item.BATCH_NO }} ({{ item.BATCH_FREEZE_QTY }})</option>' +
                '</select>'
        },
        {
            name: 'BATCH_NO', field: 'BATCH_NO', displayName: 'Batch No', visible: false, enableFiltering: true, width: '15%', cellTemplate:
                '<input type="text" disabled style="text-align: right;"  ng-model="row.entity.BATCH_NO"  class="pl-sm" />'
        ...
        {
            name: 'BATCH_FREEZE_QTY', field: 'BATCH_FREEZE_QTY', displayName: 'Freeze Qty', enableFiltering: true, visible: true, width: '10%', cellTemplate:
                '<input type="number" style="text-align: right;" disabled ng-model="row.entity.BATCH_FREEZE_QTY"  class="pl-sm" />'              
        },    

        {
            name: 'BATCH_UN_FREEZE_QTY', field: 'BATCH_UN_FREEZE_QTY', displayName: 'Unfreeze Qty', enableFiltering: true, visible: true, width: '10%', cellTemplate:
    ...
    $scope.gridBatchFreezing.rowTemplate = "<div  ng-style='row.entity.BATCH_FREEZE_QTY <row.entity.BATCH_UN_FREEZE_QTY && grid.appScope.myObj' ng-repeat=\"(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name\" class=\"ui-grid-cell\" ng-class=\"{ 'ui-grid-row-header-cell': col.isRowHeader }\" ui-grid-cell></div>"
    ...
    $scope.OnSelectBatchNo = function (entity) {
        ...
        const searchIndex = $scope.Batches.findIndex((x) => x.FREEZE_DTL_ID == entity.FREEZE_DTL_ID );
        if (searchIndex != -1) {
            entity.BATCH_ID = $scope.Batches[searchIndex].BATCH_ID;
            entity.SKU_ID = $scope.Batches[searchIndex].SKU_ID;
            entity.BATCH_NO = $scope.Batches[searchIndex].BATCH_NO;
            ...
            entity.BATCH_FREEZE_QTY = $scope.Batches[searchIndex].BATCH_FREEZE_QTY;
    ...
    $scope.addDefaultRow = (entity) => {
        ...
        var newRow = {
            ...
            BATCH_FREEZE_QTY: $scope.gridBatchFreezing.data[0].BATCH_FREEZE_QTY,
    ...
    $scope.addNewRow = (entity) => {
        ...
        if (entity.BATCH_FREEZE_QTY < entity.BATCH_UN_FREEZE_QTY) {
            notificationservice.Notification("Unfreeze qty is greater then freeze qty", "", 'Unfreeze qty is greater then freeze qty');
            return;
        }
    ...
    $scope.EditItem = (entity) => {
        ...
        if ($scope.gridBatchFreezing.data.length > 0) {
            ...
            var newRow = {
                ...
                BATCH_FREEZE_QTY: entity.BATCH_FREEZE_QTY,
    ...
    $scope.SaveData = function (model) {
        ...
        for (var i = 0; i < $scope.model.batchFreezingDtlList.length; i++) {
            if ($scope.model.batchFreezingDtlList[i].BATCH_FREEZE_QTY > $scope.model.batchFreezingDtlList[i].STOCK_QTY && $scope.model.batchFreezingDtlList[i].STATUS == "Yes") {
        ...
        BatchUnFreezingServices.InsertOrUpdate(model).then(function (data) {
    ...
    $scope.GeMstData = function () {
        ...
        BatchUnFreezingServices.GetMstData($scope.model.COMPANY_ID, $scope.model.UNIT_ID).then(function (data) {
    ...
    $scope.GetDtlData = function () {
        ...
        if ($scope.model.MST_ID > 0) {
            ...
            BatchUnFreezingServices.GetDtlData($scope.model.MST_ID).then(function (data) {
