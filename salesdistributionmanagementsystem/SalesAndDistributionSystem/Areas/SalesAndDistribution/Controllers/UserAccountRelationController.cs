...
namespace SalesAndDistributionSystem.Areas.SalesAndDistribution.Controllers
{
    ...
    public class UserAccountRelationController : Controller
    {
        private readonly IUserAccountRelationManager _service;
        private readonly ICommonServices _comservice;

        private readonly ILogger<UserAccountRelationController> _logger;
        private readonly IConfiguration _configuration;
        private readonly ServiceProvider Provider = new ServiceProvider();

        public UserAccountRelationController(ICommonServices comservice, IUserAccountRelationManager service, ILogger<UserAccountRelationController> logger, IConfiguration configuration)
        {
        ...
        public IActionResult InsertOrEdit(string Id = "0")
        {
            ...
            if (Id != "0" || Id != "")
            {
                division_Region_Mst.USER_ACCOUNT_MST_ID_ENCRYPTED = Id;
            }
        ...
        [HttpPost]
        public async Task<string> GetEditDataById([FromBody] User_Account_Relation_Mst mst)
        {
            User_Account_Relation_Mst division_Region_Mst = new User_Account_Relation_Mst();
        ...
        [HttpPost]
        public async Task<string> LoadData_Master([FromBody] User_Account_Relation_Mst division_Info)
        {
        ...
        [HttpPost]
        public async Task<string> Existing_User_Load([FromBody] User_Account_Relation_Mst division_Info)
        {

            int comp_id = division_Info == null || division_Info.COMPANY_ID == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : division_Info.COMPANY_ID;
            return await _service.Existing_User_Load(GetDbConnectionString(), comp_id);

        }
        [HttpPost]
        public async Task<string> Existing_Account_Load([FromBody] User_Account_Relation_Mst division_Info)
        {

            int comp_id = division_Info == null || division_Info.COMPANY_ID == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : division_Info.COMPANY_ID;
            return await _service.Existing_Account_Load(GetDbConnectionString(), comp_id);

        }
        [HttpPost]
        public async Task<string> LoadData_Detail([FromBody] User_Account_Relation_Dtl division_Info) => await _service.LoadData_DetailByMasterId(GetDbConnectionString(), division_Info.USER_ACCOUNT_MST_ID);


        [HttpPost]
        public async Task<JsonResult> AddOrUpdate([FromBody] User_Account_Relation_Mst model)
        {
            string result = "";
            if (model == null)
            {
