ngApp.controller('ngGridCtrl', ['$scope', 'InsertOrEditServices', 'permissionProvider', 'notificationservice', 'gridregistrationservice', '$http', '$log', '$filter', '$timeout', '$interval', '$q', function ($scope, InsertOrEditServices, permissionProvider, notificationservice, gridregistrationservice, $http, $log, $filter, $timeout, $interval, $q) {
    ...
    $scope.addNewRow = () => {
        ...
        if ($scope.gridOptions.data.length > 0 && $scope.gridOptions.data[0].CUSTOMER_CODE != null && $scope.gridOptions.data[0].CUSTOMER_CODE != '' && $scope.gridOptions.data[0].CUSTOMER_CODE != 'undefined') {
            ...
            notificationservice.Notification("Please Enter Valid Customer First", "", 'Only Single Row Left!!');
        }
    ...
    $scope.gridOptions.columnDefs = [
        ...
        , {
            name: 'CUSTOMER_CODE', field: 'CUSTOMER_CODE', pinnedLeft: true, displayName: 'Customer', enableFiltering: true, width: '18%', cellTemplate:
        ...
        , {
            name: 'INVOICE_NO', field: 'INVOICE_NO', headerCellTemplate: '<div class="">Invoice<br>No</div>', enableFiltering: false, width: '8%', cellTemplate:
        ...
        , {
            name: 'COLLECTION_AMT', field: 'COLLECTION_AMT', headerCellTemplate: '<div class="">Collection<br>Amount</div>',  enableFiltering: false, width: '8%', cellTemplate:
                '<input type="number" min="0" ng-disabled="row.entity.ROW_NO !=0" ng-model="row.entity.COLLECTION_AMT" ng-change="grid.appScope.onChangeCollectionAmt(row.entity)" class="pl-sm" />'
        }
        ...
        , {
            name: 'REMARKS', field: 'REMARKS', displayName: 'Remarks', enableFiltering: false, width: '8%', cellTemplate:
                '<input type="text"   ng-disabled="row.entity.ROW_NO !=0" ng-model="row.entity.REMARKS"  class="pl-sm" />'
        }

        , {
            name: 'Action', displayName: 'Action', width: '10%', pinnedRight: true, enableFiltering: false, enableColumnMenu: false, cellTemplate:
    ...
    $scope.onChangeCollectionAmt = function (entity) {
        var dt = $scope.gridOptions.data.filter(x => x.CUSTOMER_ID == entity.CUSTOMER_ID && x.ROW_NO != 0);
        if (dt.length > 0) {
            var tds_previous = 0;
    ...
    $scope.ClearForm = function () {
        window.location = "/SalesAndDistribution/Collection/InsertOrEdit";
    }
