...
namespace SalesAndDistributionSystem.Areas.Inventory.Controllers
{
    ...
    {
        private readonly IRequisitionReturnManager _service;
        private readonly ILogger<RequisitionReturnController> _logger;
        private readonly IConfiguration _configuration;
        private readonly ServiceProvider Provider = new ServiceProvider();
        private readonly ICommonServices _comservice;
        public RequisitionReturnController(ICommonServices comservice, IRequisitionReturnManager service, ILogger<RequisitionReturnController> logger, IConfiguration configuration)
        {
        ...
        public IActionResult RequisitionReturn(string Id = "0")
        {
            ...
            DEPOT_REQUISITION_RETURN_MST requisitionMst = new DEPOT_REQUISITION_RETURN_MST();
        ...
        [HttpPost]
        public async Task<JsonResult> AddOrUpdate([FromBody] DEPOT_REQUISITION_RETURN_MST model)
        {
            string result = "";
            ...
            else
            {
                try
                {
                    if (model.MST_ID == 0)
                    {
                        ...
                        model.RETURN_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
                        model.RETURN_UNIT_ID = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UnitId)?.Value;
                        model.RETURN_BY = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UserId)?.Value;
        ...
        [HttpPost]
        public async Task<string> LoadDataForRcv(int CompanyId)
        {
            var unitId = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UnitId)?.Value;
            int comp_id = CompanyId == null || CompanyId == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : CompanyId;
            return await _service.LoadDataForRcv(GetDbConnectionString(), comp_id, unitId);
        }

        [HttpPost]
        public async Task<string> UnreturnedRequisition(int CompanyId)
        {
            var unitId = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UnitId)?.Value;
            int comp_id = CompanyId == null || CompanyId == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : CompanyId;
            return await _service.UnreturnedRequisition(GetDbConnectionString(), comp_id, User.GetUnitId());
        }
