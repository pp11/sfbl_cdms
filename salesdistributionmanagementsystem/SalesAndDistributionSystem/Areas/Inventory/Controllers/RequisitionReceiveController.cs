...
{
    [Area("Inventory")]
    public class RequisitionReceiveController : Controller
    {
        private readonly IRequisitionReceiveManager _service;
        private readonly ILogger<RequisitionReceiveController> _logger;
        private readonly IConfiguration _configuration;
        private readonly ServiceProvider Provider = new ServiceProvider();
        private readonly ICommonServices _comservice;
        private string GetDbConnectionString() => Provider.GetConnectionString(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value.ToString(), "SalesAndDistribution");
        public RequisitionReceiveController(ICommonServices comservice, IRequisitionReceiveManager service, ILogger<RequisitionReceiveController> logger, IConfiguration configuration)
        {
        ...
        public IActionResult RequisitionRcv(string Id = "0")
        {
            ...
            DEPOT_REQUISITION_RCV_MST requisitionMst = new DEPOT_REQUISITION_RCV_MST();
            requisitionMst.MST_ID_ENCRYPTED = Id;
            return View(requisitionMst);
        }

        [HttpPost]
        public async Task<string> GetEditDataById([FromBody] DEPOT_REQUISITION_RCV_MST mst)
        {
        ...
        [HttpPost]
        public async Task<string> GetRcvDtlDataById([FromBody] DEPOT_REQUISITION_RCV_MST mst)
        {
            DEPOT_REQUISITION_RCV_MST requisition_Mst = new DEPOT_REQUISITION_RCV_MST();
            if (mst.q != null && mst.q != "0" && mst.q != "")
            {
                int _Id = Convert.ToInt32(_comservice.Decrypt(mst.q));
                requisition_Mst = await _service.LoadDetailDataWithStock_ByMasterId_List(GetDbConnectionString(), _Id);

            }
            return JsonSerializer.Serialize(requisition_Mst);
        }
        [HttpPost]
        public async Task<JsonResult> AddOrUpdate([FromBody] DEPOT_REQUISITION_RCV_MST model)
        {
            string result = "";
            ...
            else
            {
                try
                {
                    if (model.MST_ID == 0)
                    {
                        ...
                        model.RECEIVE_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
           
                        model.RECEIVE_BY = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UserId)?.Value;
                        model.ISSUE_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
                        model.ENTERED_BY = User.Claims.FirstOrDefault(c => c.Type == ClaimsType.UserId)?.Value;
                        model.ENTERED_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt");
                        model.ENTERED_TERMINAL = HttpContext.Connection.RemoteIpAddress.ToString();
                        model.COMPANY_ID = model.COMPANY_ID == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : model.COMPANY_ID;
                        if (model != null && model.requisitionRcvDtlList.Count > 0)
                        {
        ...
        [HttpPost]
        public async Task<string> LoadReceiveData(int CompanyId)
        {
            int comp_id = CompanyId == null || CompanyId == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : CompanyId;
            return await _service.LoadData(GetDbConnectionString(), comp_id);
        }
