ngApp.controller('ngGridCtrl', ['$scope', 'StockTransferRcvService', 'permissionProvider', 'notificationservice', 'gridregistrationservice', '$http', '$log', '$filter', '$timeout', '$interval', '$q', function ($scope, stockTransferRcvService, permissionProvider, notificationservice, gridregistrationservice, $http, $log, $filter, $timeout, $interval, $q) {
    ...
    $scope.gridOptionsList.columnDefs = [
        ...
        , {
            name: 'SKU_NAME', field: 'SKU_NAME', displayName: 'SKU Name', enableFiltering: true, width: '28%', cellTemplate:
                '<input type="text" disabled  style="text-align: left;"  ng-model="row.entity.SKU_NAME"  class="pl-sm" />'

        }, {
            name: 'SKU_ID', field: 'SKU_NAME', displayName: 'SKU id', enableFiltering: true, width: '20%', visible: false, cellTemplate:
                '<input type="text" disabled  style="text-align: left;"  ng-model="row.entity.SKU_ID"  class="pl-sm" />'

        },
        {
            name: 'SKU_CODE', field: 'SKU_CODE', displayName: 'SKU Code', enableFiltering: true, width: '100', cellTemplate:
                '<input type="text" disabled  style="text-align: ledt;"  ng-model="row.entity.SKU_CODE"  class="pl-sm" />'
        }
        , {
            name: 'UNIT_TP', field: 'UNIT_TP', displayName: 'Unit TP', enableFiltering: true, width: '80', cellTemplate:
                '<input type="number" disabled style="text-align:right;" disabled  ng-model="row.entity.UNIT_TP"  class="pl-sm" />'
        }
        , {
            name: 'PACK_SIZE', field: 'PACK_SIZE', displayName: 'PACK Size', enableFiltering: true, width: '90', cellTemplate:
                '<input type="text" disabled style="text-align:right;" disabled  ng-model="row.entity.PACK_SIZE"  class="pl-sm" />'
        }

        , {
            name: 'TRANSFER_QTY', field: 'TRANSFER_QTY', displayName: 'Transfer Qty', enableFiltering: true, width: '110', cellTemplate:
        ...
        {
            name: 'TRANS_RCV_QTY', field: 'TRANS_RCV_QTY', displayName: 'Receive Qty', enableFiltering: true, width: '110', cellTemplate:
        ...
        {
            name: 'TRANS_RCV_AMOUNT', field: 'TRANS_RCV_AMOUNT', displayName: 'Receive Amount', enableFiltering: true, width: '120', cellTemplate:
    ...
    $scope.ClearForm = function () {
        window.location.href = "/Inventory/StockTransferRcv/StockTransferRcv";
    }
    ...
    $scope.LoadTransferData = function () {
        $scope.showLoader = true;
        stockTransferRcvService.LoadDispatchedTransferData($scope.model.COMPANY_ID).then(function (data) {
    ...
    $scope.LoadTransferDtlData = function () {
        $scope.showLoader = true;
        let selectedOption = JSON.parse($scope.model.TRANSFER_OBJ);
        $scope.model.TRANSFER_AMOUNT = selectedOption.TRANSFER_AMOUNT;
        $scope.model.TRANSFER_DATE = selectedOption.TRANSFER_DATE;
        $scope.model.TRANSFER_UNIT_ID = selectedOption.TRANSFER_UNIT_ID.toString();
        $scope.model.TRANSFER_NO = selectedOption.TRANSFER_NO;
        $scope.model.DISPATCH_NO = selectedOption.DISPATCH_NO;
        
        stockTransferRcvService.LoadDispatchedTransferDtl(selectedOption.COMPANY_ID, selectedOption.TRANS_RCV_UNIT_ID, selectedOption.DISPATCH_NO, selectedOption.TRANSFER_NO).then(function (data) {
    ...
    //    stockTransferRcvService.GetTransferDtlDataById('0cjaRvXB2OS5e9smj74MU9U5nA/7yP2bbh7VmA4yNA6e0ZfU4bUHundek27H2sW8').then(function (data) {


    //        $scope.gridOptionsList.data = data.data.stockTransferDtlList;;
    //        console.log(data.data.stockTransferDtlList)
