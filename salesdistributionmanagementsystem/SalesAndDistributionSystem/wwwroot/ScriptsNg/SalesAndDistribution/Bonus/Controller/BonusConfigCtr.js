ngApp.controller('ngGridCtrl', ['$scope', 'BonusConfigServices', 'permissionProvider', 'notificationservice', 'gridregistrationservice', '$http', '$log', '$filter', '$timeout', '$interval', '$q', function ($scope, BonusConfigServices, permissionProvider, notificationservice, gridregistrationservice, $http, $log, $filter, $timeout, $interval, $q) {
    ...
    $scope.addNewRow = () => {
        ...
        if ($scope.gridBonusList.data.length > 0 && $scope.gridBonusList.data[0].SLAB_QTY != null && $scope.gridBonusList.data[0].SLAB_QTY != '' && $scope.gridBonusList.data[0].SLAB_QTY != 'undefined' && $scope.gridBonusList.data[0].CALCULATION_TYPE != '' && $scope.gridBonusList.data[0].CALCULATION_TYPE != 'undefined') {
            ...
            if ($scope.gridBonusList.data[0].BONUS_TYPE == 'Disc_%') {
                if ($scope.gridBonusList.data[0].DISCOUNT_PCT <= 0) {
                    errormsg = errormsg + " Please Enter discount Percent";
                    flag = false;
                }
            }
            else if ($scope.gridBonusList.data[0].BONUS_TYPE == 'Disc_Value') {
                if ($scope.gridBonusList.data[0].DISCOUNT_VAL <= 0) {
                    errormsg = errormsg + " Please Enter discount value";
                    flag = false;
                }
            }
            else if ($scope.gridBonusList.data[0].BONUS_TYPE == 'SKU') {
                if ($scope.gridBonusList.data[0].BONUS_SKU_ID <= 0 || $scope.gridBonusList.data[0].BONUS_QTY <= 0) {
                    errormsg = errormsg + " Please Select Product and Quantity First!";
                    flag = false;
                }
            }
            else if ($scope.gridBonusList.data[0].BONUS_TYPE == 'Gift') {
            ...
            if (flag == true) {
                var newRow = {
                    ...
                    BONUS_SKU_ID: $scope.gridBonusList.data[0].BONUS_SKU_ID,
                    BONUS_DTL_ID: $scope.gridBonusList.data[0].BONUS_DTL_ID,
                    BONUS_MST_ID: $scope.gridBonusList.data[0].BONUS_MST_ID,
                    BONUS_SLAB_TYPE: $scope.gridBonusList.data[0].BONUS_SLAB_TYPE,
                    SLAB_QTY: $scope.gridBonusList.data[0].SLAB_QTY,
                    BONUS_TYPE: $scope.gridBonusList.data[0].BONUS_TYPE,
                    BONUS_QTY: $scope.gridBonusList.data[0].BONUS_QTY,
    ...
    $scope.gridBonusList.columnDefs = [
        ...
        , {
            name: 'BONUS_SLAB_TYPE', field: 'BONUS_SLAB_TYPE', displayName: 'Slab Type', enableFiltering: true, width: '7%', cellTemplate:
                '<select class="form-control" id="BONUS_SLAB_TYPE" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_DTL_ID>0"' +
        ...
        , {
            name: 'BONUS_TYPE', field: 'BONUS_TYPE', displayName: 'Bonus Type', enableFiltering: true, width: '8%', cellTemplate:
                '<select class="form-control" id="BONUS_TYPE_TT" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_DTL_ID>0"' +
        ...
        , {
            name: 'DISCOUNT_PCT', field: 'DISCOUNT_PCT', displayName: 'Discount %', enableFiltering: true, width: '8%', cellTemplate:
                '<input type="number" min="0"  ng-model="row.entity.DISCOUNT_PCT" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_TYPE != \'Disc_%\'"   class="pl-sm" />'
        }
        , {
            name: 'DISCOUNT_VAL', field: 'DISCOUNT_VAL', displayName: 'Discount Val', enableFiltering: true, width: '8%', cellTemplate:
                '<input type="number" min="0"  ng-model="row.entity.DISCOUNT_VAL" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_TYPE != \'Disc_Value\'"   class="pl-sm" />'
        }
        , {
        ...
        , {
            name: 'BONUS_SKU_CODE', field: 'BONUS_SKU_CODE', displayName: 'Code', enableFiltering: true, width: '12%', visible: false,  cellTemplate:
        ...
        , {
            name: 'BONUS_QTY', field: 'BONUS_QTY', displayName: 'Bonus Qty', enableFiltering: true, width: '7%', cellTemplate:
                '<input type="number" min="0"  ng-model="row.entity.BONUS_QTY" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_TYPE != \'SKU\'"   class="pl-sm" />'
        }
        , {
        ...
        , {
            name: 'GIFT_QTY', field: 'GIFT_QTY', displayName: 'Gift Qty', enableFiltering: true, width: '9%', cellTemplate:
                '<input type="number" min="0"  ng-model="row.entity.GIFT_QTY" ng-disabled="row.entity.ROW_NO !=0 || row.entity.BONUS_TYPE != \'Gift\'"   class="pl-sm" />'
        }
                ...
                if (data.data != null && data.data.Bonus_Dtls != null && data.data.Bonus_Dtls.length > 0) {
                    ...
                    if (data.data.Bonus_Dtls != null) {
                        ...
                        for (var i in data.data.Bonus_Dtls) {
                            data.data.Bonus_Dtls[i].BONUS_SKU_ID = JSON.stringify(data.data.Bonus_Dtls[i].BONUS_SKU_ID);
    ...
    $scope.SaveData = function (model) {
        ...
        for (var i in $scope.gridBonusList.data) {
            ...
            const searchIndex = $scope.Products.findIndex((x) => x.SKU_ID == $scope.gridBonusList.data[i].BONUS_SKU_ID);
            if (searchIndex != -1) {
                $scope.gridBonusList.data[i].BONUS_SKU_ID = $scope.Products[searchIndex].SKU_ID;
                $scope.gridBonusList.data[i].BONUS_SKU_CODE = $scope.Products[searchIndex].SKU_CODE;
